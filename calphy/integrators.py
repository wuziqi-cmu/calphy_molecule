...
#Constants
h = const.physical_constants["Planck constant in eV/Hz"][0] * 23.06035 #convert to kcal/mol when using "units real" in lammps
hbar = h/(2*np.pi)
kb = const.physical_constants["Boltzmann constant in eV/K"][0] * 23.06035 #convert to kcal/mol when using "units real" in lammps
kbJ = const.physical_constants["Boltzmann constant"][0]
Na = const.physical_constants["Avogadro constant"][0]
eV2J = const.eV
kcalpMol2J = 6.94770014E-21
...
def get_einstein_crystal_fe(temp, natoms, mass, vol, k, concentration, cm_correction=True):
    ...
    totalMass = mass.dot(concentration)*natoms
    #convert k from ev/A2 to J/m2
    # k = np.array(k)*(eV2J/1E-20)
    k = np.array(k)*(kcalpMol2J/1E-20)
    ...
    conversion = 1.6021766344E-19/23.06035

    print(h*conversion, kb*conversion)

    for count, spices in enumerate(zip(mass, k)):

        LAMBDA = np.sqrt((h * conversion) ** 2 / (2 * np.pi * spices[0] * kb * conversion * temp))

        part1 += natoms * concentration[count] * 3 / 2 * np.log(
            np.square(LAMBDA) * spices[1] / (2 * np.pi * kb * conversion * temp))
        part2point5 += 2 * np.pi * kb * conversion * temp * natoms * concentration[count] * (
                    spices[0] / totalMass) ** 2 / spices[1]

    part2 = 3/2*np.log(part2point5)
    part3 = np.log(natoms/3/vol)

    F_harm = kb*conversion*temp*(part1 + part2 + part3)/natoms*3
    F_harm = F_harm*1e-3*Na
    print(F_harm)
    ...
    #     F_harm += concentration[count]*np.log((hbar*om)/(kb*temp))#*natons
    #     if cm_correction:
    ...
    return F_harm*0.239006 # convert to kcal/mol from kJ/mol
...
def integrate_rs(simfolder, f0, t, natoms, p=0, nsims=5, 
    ...
    for i in range(1, nsims+1):
        ...
        fvol = fvol/natoms
        bvol = bvol/natoms
        fdx = fdx + p*fvol * 1E-30 * 1.439327784E20
        bdx = bdx + p*bvol * 1E-30 * 1.439327784E20
    ...
    f = f0/flambda + 1.5*kb*temp*np.log(flambda)/flambda + wmean
