...
class Phase:
    ...
    def prepare_lattice(self):
        ...
        """
        l, alat, apc, conc = pl.prepare_lattice(self.calc, self.options["units"], self.options["atom_style"])
        self.l = l
        self.alat = alat
        self.apc = apc
    ...
    def reversible_scaling(self, iteration=1):
        ...
        lmp.command("dump dumpxyz all custom 1 tip4p_rs_traj.dump type x y z")
        ...
        for pc in self.options["md"]["pair_coeff"]:
            pcraw = pc.split()
            pcnew = " ".join([*pcraw[:2], self.options["md"]["pair_style"].split()[0], "1", *pcraw[2:]])
            lmp.pair_coeff(pcnew)
            pcnew = " ".join([*pcraw[:2], self.options["md"]["pair_style"].split()[0], "2", *pcraw[2:]])
            lmp.pair_coeff(pcnew)
